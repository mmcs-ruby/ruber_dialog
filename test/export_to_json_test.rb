...
class ExportToJsonTest < Minitest::Test
  ...
  def test_node_to_json
    lines = [Line.new("Vincenzo", "Please, listen to me."),
             Line.new("Cha Young", "Oh no..."),
             Line.new("Vincenzo", "I'm the mafia.")]

    responses = [Response.new("I believe you.", "Telling the truth"),
                 Response.new("Good joke.", "End"),
                 Response.new("In the morning?", "Talking about the song")]

    node = Node.new("Confession", lines, responses)

    json = { name: "Confession",
             lines: [{ char_name: "Vincenzo", phrase: "Please, listen to me." },
  ...
  def test_final_node_to_json
    final_lines = [Line.new("Vincenzo", "Well, it was a long journey."),
                   Line.new("Cha Young", "That's true. I think we have to drink some beer."),
                   Line.new("Vincenzo", "And maybe some Italian vine...")]

    final_node = Node.new("End", final_lines, nil)

    final_json = { name: "End",
                   lines: [{ char_name: "Vincenzo", phrase: "Well, it was a long journey." },
                           { char_name: "Cha Young", phrase: "That's true. I think we have to drink some beer." },
                           { char_name: "Vincenzo", phrase: "And maybe some Italian vine..." }],
                   responses: nil }.to_json
  ...
  def test_dialog_to_json
    ...
    dialog = Dialog.new(starting_node,
                        [node_1, node_2, node_3, node_4],
                        [vincenzo, inzaghi],
                        ["Fun story", "Exciting story", "Well..."])
    ...
    json = { starting_node: { name: starting_node.name, lines: starting_node.lines, responses: starting_node.responses },
             nodes: [{ name: node_1.name, lines: node_1.lines, responses: node_1.responses },
                     ...
                     { name: node_4.name, lines: node_4.lines, responses: node_4.responses }],
             characters: [{ name: vincenzo.name },
                          { name: inzaghi.name }],
             final_nodes_names: [node_1.name, node_3.name, node_4.name] }.to_json
